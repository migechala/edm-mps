...
def calculate_inception_stats(
    ...
    mu = torch.zeros([feature_dim], dtype=torch.float32, device=device)
    sigma = torch.zeros([feature_dim, feature_dim], dtype=torch.float32, device=device)
    for images, _labels in tqdm.tqdm(data_loader, unit='batch', disable=(dist.get_rank() != 0)):
        ...
        features = detector_net(images.to(device), **detector_kwargs).to(torch.float32)
        mu += features.sum(0)
